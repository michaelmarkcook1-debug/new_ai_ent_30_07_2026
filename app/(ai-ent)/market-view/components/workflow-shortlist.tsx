      ...
      {vendors.slice(0, limit).map((v, i) => (
        <li key={v.vendorId} className="flex flex-wrap items-center gap-2">
          ...
          <Link
            ...
            className="min-w-[7rem] text-[12.5px] font-semibold hover:text-primary hover:underline"
          >
            {v.name}
          </Link>
          <span className="rounded-full border border-base-300 px-2 py-0.5 text-[10px] text-muted">
  ...
  return (
    <section className="rounded-lg border border-base-300 bg-base-100 p-4">
      <div className="flex flex-wrap items-start justify-between gap-2">
        ...
        <div className="flex flex-wrap items-center gap-1">
          {shortlist.mappedCategories.map((c) => (
            <span
              key={c.id}
              className="rounded-full bg-base-200 px-2 py-0.5 text-[10px] text-muted"
            >
              {c.name}
            </span>
          ))}
      ...
      <div className="mt-3 grid grid-cols-1 gap-4 @2xl:grid-cols-2">
        ...
        <div>
          <p className="text-[12px] font-bold">Build it</p>
          <p className="text-[11px] text-muted">
