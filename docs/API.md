...
Prepares every page that authors an analyst reading, so no reader pays for a
model call. For the scheduler only.

**Authentication.** `Authorization: Bearer <CRON_SECRET>`. Anything else is
`401` with `code` `NOT_SCHEDULER`; when `CRON_SECRET` is not configured every
request is `401` with `code` `CRON_SECRET_UNSET`. The `x-vercel-cron` header
is not consulted.

**Behaviour.** Fetches the pages in `lib/analyst/warm-list.ts` through a pool
of `WARM_CONCURRENCY` (4) workers, each page aborted at 150 seconds, no page
started after 240 seconds have elapsed. Never calls the model itself.

**Response.** `200` when every target was fetched and none failed or timed out;
`503` otherwise, with the same body.
...
```json
{
  ...
  "totalMs": 131842,
  "results": [
    { "path": "/pulse", "outcome": "authored", "status": 200, "ms": 30211 }
  ]
}
```

`outcome` is one of `authored` (written during this request), `cached`
(written, served in under 5 seconds), `fallback` (computed reading), `failed`
(non-200, or a page with no reading), `timed-out`, or the page appears in
`remainingPaths` because the budget ran out before it was started. Figures in
the example are illustrative of shape only.
